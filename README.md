...
Kaggle 대회 이름만 지정해도 되지만, 샘플문항 톤을 함께 적어주면 결과가 더 안정적입니다.

```text
$kaggle-aice-associate-builder
titanic 데이터를 기준으로
AICE Associate 스타일 노트북을 만들어줘.
샘플문항 톤으로 작성해줘.
```

### 2. 회귀 문제 지정

문제 유형과 상단 구성 요소를 명시하면 샘플에 더 가깝게 나옵니다.

```text
$kaggle-aice-associate-builder
house-prices-advanced-regression-techniques 대회를 기준으로
AICE Associate 스타일 문제지/해설지 노트북을 만들어줘.
샘플문항처럼 상단에 유의사항, 도메인, 목표, 시나리오, 컬럼 설명을 넣어줘.
```
...
분류 문제용으로 만들고, 문체는 샘플문항처럼 짧고 시험지 스타일로 작성해줘.
```
...
> - `샘플문항 톤으로`, `상단에 유의사항/도메인/목표/시나리오/컬럼 설명을 넣어줘`를 함께 쓰면 결과 형식이 더 잘 맞습니다.
