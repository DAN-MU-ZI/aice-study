...
AI 에이전트에게 Kaggle 대회 이름만 알려주면, 데이터 분석 → 전처리 → 모델링까지 구성된 실습 노트북(`.ipynb`)과 자동 채점 스크립트가 한 번에 만들어집니다.
...
| 대상 | 이 환경을 쓰면 좋은 이유 |
|------|--------------------------|
| 🎯 **AICE Associate 수험생** | 실제 시험과 동일한 14문항 구성으로 실전 감각을 키울 수 있습니다 |
| 🐍 **Python·ML 입문자** | Kaggle 데이터로 EDA → 전처리 → 모델링의 전체 흐름을 한 노트북에서 체험합니다 |
| 👨‍🏫 **AI 교육 강사** | 대회 이름만 바꿔 즉시 새 실습 자료를 생성할 수 있어 강의 준비 시간을 줄입니다 |
| 🔄 **반복 실습이 필요한 학습자** | 다양한 데이터셋으로 같은 형식의 문제를 무한히 생성해 패턴을 체득합니다 |
...
실제 시험과 동일한 4가지 유형으로 출제됩니다:

| 유형 | 설명 |
|------|------|
| **A. 직접 코딩** | 지시사항을 읽고 빈 코드 셀에 직접 작성 |
| **B. 오류 정정** | 의도적 오타가 포함된 코드를 수정 |
| **C. 빈칸 채우기** | 코드 내 `<#N-1>` 빈칸에 들어갈 코드 작성 |
| **D. 결과 해석** | 시각화·계산 결과를 보고 단답형 응답 |
...
└── <대회명>/
    ...
    └── tests/
        └── validate_notebook.py  ← 자동 채점 스크립트
```
...
```text
$kaggle-aice-associate-builder titanic 데이터를 기준으로 AICE Associate 스타일 14문항 노트북을 만들어줘.
```

### 2. 대회 + 폴더 구조 + 테스트까지 한번에

```text
$kaggle-aice-associate-builder kaggle의 house-prices-advanced-regression-techniques 대회를 기준으로 data/raw, data/submissions, tests/validate_notebook.py까지 포함한 실습 세트를 구성해줘.
```

### 3. 분류 문제 지정 + 한국어 + 자동 채점

```text
$kaggle-aice-associate-builder heart-disease-uci 데이터셋을 찾아서 분류 문제용 노트북을 한국어로 작성하고, answer_* 변수를 검증하는 테스트도 만들어줘.
```
...
```text
$kaggle-aice-associate-builder 먼저 Kaggle 데이터 구조를 조사하고, 그 결과를 바탕으로 14문항 구성안만 초안으로 보여줘.
```

### 5. 제출 파일(submission.csv)까지 완성

```text
$kaggle-aice-associate-builder bike-sharing-demand 데이터를 기준으로 회귀 문제 실습 세트를 만들고, 마지막에 submission.csv 예시까지 생성해줘.
```

> [!TIP]
> **지시를 더 정확하게 쓰는 팁**
> - 분류인지 회귀인지 명시하면 더 적합한 문제가 구성됩니다.
> - 한국어/영어 여부를 적으면 노트북 언어가 통일됩니다.
> - 원하는 산출물을 구체적으로 나열하세요: `.ipynb`, `tests/`, `submission.csv` 등
