...
12. **데이터 획득은 Kaggle MCP 우선** — 데이터가 워크스페이스에 아직 없으면, 임의의 셸 다운로드나 수동 URL 추측 전에 Kaggle MCP로 대회/데이터셋 메타데이터와 파일 목록을 먼저 조회하고 가능한 경우 그 경로로 내려받습니다. MCP 접근이 막혀 있으면 그 사실을 명시한 뒤에만 대체 경로를 검토합니다.
